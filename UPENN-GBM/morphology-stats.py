...
os.makedirs(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/",figs_folder), exist_ok=True)

morphology = pd.read_csv(os.path.join(args.path, f"TDMaps_IDH1-WT/morphology-tissues.csv"))
morphology.rename(columns={"Survival_from_surgery_days_UPDATED": "OS"}, inplace=True)
morphology.rename(columns={"Survival_Status": "1-dead 0-alive"}, inplace=True)
...
fig.tight_layout()
fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/correlation-morphology.{args.format}"), dpi=300, format=args.format)
plt.close()
...
for status in [0,1]:
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/OS-morphology_scatter_status-{status}.{args.format}"), dpi=300, format=args.format)
    plt.close()
...
fig.tight_layout()
fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Survival-morphology_step-monthly.{args.format}"), dpi=300, format=args.format)
plt.close()
...
for i in range(1,len(morphology_tissues.columns)):
    ...
    if (i-1)==0:
        ax[i-1].legend(frameon=True, ncols=1, loc="upper right")
fig.tight_layout()
fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Survival-morphology_step-monthly_status-1.{args.format}"), dpi=300, format=args.format)
plt.close()
...
for p_iter, (plow, phigh) in enumerate(percentiles2check):
    ...
    for status in [1]:
        ...
        fig.tight_layout()
        if status==1:
            fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/OS-morphology_death-cutoff_status-{status}_p-{perc}.{args.format}"), dpi=300, format=args.format)
        else:
            fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/OS-morphology_dropout-cutoff_status-{status}_p-{perc}.{args.format}"), dpi=300, format=args.format)
        plt.close()
    ...
    for i in range(1,len(morphology_tissues.columns)):
        ...
        ax[i-1].spines[["top", "right"]].set_visible(False)
    fig.tight_layout()
    fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/OS-morphology_percentiles-{plow}-{phigh}_status-1.{args.format}"), dpi=300, format=args.format)
    plt.close()
...
with open(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/stats-featuresMORPH_CoxPHazard.txt"), "w") as stats_file:
    stats_file.write(stats_string)
...
for model in ["linear","rbf"]:# 
    for plow, phigh in [(25,75),(50,50)]:
        ...
        fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Survival-prediction_model-{model}SVC_percvolume-{plow}.{args.format}"), dpi=300, format=args.format)
        fig_bis.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Survival-prediction_model-{model}SVC_percvolume_significance-{plow}.{args.format}"), dpi=300, format=args.format)
        plt.close(fig)
        plt.close(fig_bis)
...
for plow, phigh in [(25,75),(50,50)]:
    ...
    for i_ax, splits in enumerate(N_splits):
        ...
        if i_ax==(len(N_splits)-1):
            ...
            ax[i_ax,1].tick_params(axis='x', length=0) 
    fig.tight_layout()
    fig.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Survival-prediction_model-volume_percvolume-{plow}.{args.format}"), dpi=300, format=args.format)
    plt.close(fig)
...
for plow, phigh in percentiles2check:
    ...
    fig_roc.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Death-prediction_model-volume_p-{plow}_metric-ROC.{args.format}"), dpi=300, format=args.format)
    plt.close(fig_roc)          
    fig_auc.tight_layout()
    fig_auc.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Death-prediction_model-volume_p-{plow}_metric-AUC.{args.format}"), dpi=300, format=args.format)
    plt.close(fig_auc)          
    fig_acc.tight_layout()
    fig_acc.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Death-prediction_model-volume_p-{plow}_metric-ACC.{args.format}"), dpi=300, format=args.format)
    plt.close(fig_acc)          
    fig_bacc.tight_layout()
    fig_bacc.savefig(os.path.join(args.path, f"Figures/TDMaps_IDH1-WT/{figs_folder}/Death-prediction_model-volume_p-{plow}_metric-BACC.{args.format}"), dpi=300, format=args.format)
    plt.close(fig_bacc)      
    print("**************************")