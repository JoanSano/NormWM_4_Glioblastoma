...
if __name__ == '__main__':
    ...
    row = pd.DataFrame(demographics.loc[demographics[ID_column]==subject_ID])
    if row.empty:
    ...
    for tissue, column in keys.items():
        ...
        file = f"{args.dir}/TDMaps_IDH1-{args.idh1}/{subject_ID}/maps/{subject_ID}_{tissue}.nii.gz"
        if os.path.exists(file):
            ...
            # Average TDI --> We need to mask and only count contributions within the lesion & tissue mask
                ...
                ts = tissue.split("_")[0]
                mask = nib.load(f"{args.dir}/TDMaps_IDH1-{args.idh1}/{subject_ID}/masks/{subject_ID}_{ts}.nii.gz").get_fdata()
                if mask.sum()<1:
