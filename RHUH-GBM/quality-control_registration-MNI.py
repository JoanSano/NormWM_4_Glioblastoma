...
parser.add_argument("--modality_prefix", type=str, help="Directory where the data to collect is stored", default="RHUH-GBM_data-T0_MNI-ICBM-2009b-NLIN-ASYM")
...
parser.add_argument("--metadata", type=str, help="Name of the clinical data file", default="clinical_data_TCIA_RHUH-GBM.csv")
...
subjects = demographics["Patient ID"]

figures_dir = os.path.join(args.main_dir, "Figures", f"QC_registration-MNI_data-T{timepoint}")
...
iso_levels = {
    ...
    "flair": np.linspace(0, 1, 7),
    "t2": np.linspace(0, 1, 5),
    "t1": np.linspace(0, 1, 7),
    "t1ce": np.linspace(0, 1, 7),
}
total_s = 0
for i, s in enumerate(subjects):
    ...
    for j, m in enumerate(modalities):
        try:
            ...
            nifti_ = nib.load(file)
            if "seg" not in m:
                nifti = nib.Nifti1Image(
                    (nifti_.get_fdata()-nifti_.get_fdata().min())/(nifti_.get_fdata().max()-nifti_.get_fdata().min()) * MASK.get_fdata(),
                    nifti_.affine, 
                    nifti_.header
                )
            else:
    ...
    total_s += 1
    print(f"Subject {s}, number {total_s} from {len(subjects)} ({round(100*total_s/len(subjects), 4)}%)\n")
