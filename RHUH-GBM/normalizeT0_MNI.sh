...
##### TODO: THIS SHOULD ONLY BE USED FOR THE INITIAL TIMEPOINT! WE SHOULD CREATE A NEW SCRIPT FOR THE OTHER TIMEPOINTS WITH AFFINE REGISTRATION AND THEN APPLY THE WARPING!
 
# Define base paths
CONTRAST="T2"
BASE_DIR="/home/joan/Desktop/PROJECTS/Glioblastomas/Glioblastoma_RHUH-GBM_v2-29102025"
...
MNI_DIR="/home/joan/Desktop/PROJECTS/Glioblastomas/Glioblastoma_RHUH-GBM_v2-29102025/RHUH-GBM_data-T0_MNI-ICBM-2009b-NLIN-ASYM"
if [[ -d "$MNI_DIR" ]]; then
...
for subject in "$BASE_DIR"/RHUH-GBM_data-T0/*; do
    (
        ...
        if [[ -d "$subject" ]]; then
            ...
            if [[ -f "${OUTPUT_NAME}__1Warp.nii.gz" ]]; then
                ...
                MASK_NAME=$(find "$subject" -name "*_segmentations.nii.gz")
            ...
            if [ ${#images[@]} -eq 0 ]; then
                echo "  ${subject}: No images to apply the fitted transformation!"
            else
