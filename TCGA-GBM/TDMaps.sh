...
CLINICAL_DATA_FILE="TCGA-GBM_clinical_v3.0_IDH1-${IDH}.csv"
...
echo "patient,Gender,Age..years.at.diagnosis.,Grade,Survival..months.,Vital.status..1.dead.,IDH.status,MGMT.promoter.status,Karnofsky.Performance.Score,diagnosis.WHO2021,automatic.segmentation,manaul.correction,${MORP_LABELS}" > $MORPHOLOGY
...
for lesion in $LESION_DIR/IDH1-$IDH/*; do
    ( 
        ...
        if [[ ! -f $EH ]]; then fslmaths $lesion -thr 4 -uthr 4 -bin $EH; fi
