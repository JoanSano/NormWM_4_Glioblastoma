...
while getopts ":g:k:d:n:s:h" opt; do
  case ${opt} in
    g)
      GRADE=$OPTARG
      ;;
...
echo "ID,Sex,Age at MRI,WHO CNS Grade,Final pathologic diagnosis (WHO 2021),MGMT status,MGMT index,1p/19q,IDH,1-dead 0-alive,OS,EOR,Biopsy prior to imaging,BraTS21 ID,BraTS21 Segmentation Cohort,BraTS21 MGMT Cohort,# Labels,${MORP_LABELS}" > $MORPHOLOGY
